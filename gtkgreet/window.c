...
void on_visibility_icon_press(GtkWidget *widget, gpointer data) {
  ...
  if (visible) {
    ...
    gtk_entry_set_icon_from_icon_name(
        GTK_ENTRY(widget), GTK_ENTRY_ICON_SECONDARY, "view-reveal-symbolic");
  } else {
...
void window_setup_question(struct Window *ctx, enum QuestionType type,
  ...
  case QuestionTypeSecret: {
    ...
    if (type == QuestionTypeSecret) {
      ...
      gtk_entry_set_icon_from_icon_name(GTK_ENTRY((GtkEntry *)ctx->input_field),
                                        GTK_ENTRY_ICON_SECONDARY,
                                        "view-reveal-symbolic");
      gtk_entry_set_icon_activatable((GtkEntry *)ctx->input_field,
                                     GTK_ENTRY_ICON_SECONDARY, TRUE);
      g_signal_connect(ctx->input_field, "icon-press",
                       G_CALLBACK(on_visibility_icon_press), NULL);
    }
  ...
  if (type == QuestionTypeInitial) {
    ...
    gtk_entry_set_placeholder_text((GtkEntry *)selector_entry,
                                   _("Command to run on login"));
...
static void window_setup(struct Window *ctx) {
  ...
  if (ctx->revealer == NULL) {
    ...
    gtk_revealer_set_transition_type(GTK_REVEALER(ctx->revealer),
  ...
  if (gtkgreet->focused_window == ctx || gtkgreet->focused_window == NULL) {
    ...
    window_setup_question(ctx, gtkgreet->question_type, gtkgreet->question,
                          gtkgreet->error);
...
static void window_set_focus(struct Window *win, struct Window *old) {
  ...
  if (old != NULL && old != win) {
    if (old->input_field != NULL && win->input_field != NULL) {
      ...
      gint cursor_pos = 0;
      g_object_get((GtkEntry *)old->input_field, "cursor-position", &cursor_pos,
                   NULL);
      ...
      g_signal_emit_by_name((GtkEntry *)win->input_field, "move-cursor",
                            GTK_MOVEMENT_BUFFER_ENDS, -1, FALSE);
      g_signal_emit_by_name((GtkEntry *)win->input_field, "move-cursor",
                            GTK_MOVEMENT_LOGICAL_POSITIONS, cursor_pos, FALSE);
    }
    if (old->command_selector != NULL && win->command_selector != NULL) {
      gtk_combo_box_set_active(
          (GtkComboBox *)win->command_selector,
          gtk_combo_box_get_active((GtkComboBox *)old->command_selector));
    }
    window_setup(old);
...
static gboolean window_background(GtkWidget *widget, cairo_t *cr,
                                  gpointer data) {
...
struct Window *create_window(GdkMonitor *monitor) {
  ...
  g_signal_connect(w->window, "destroy", G_CALLBACK(window_destroy_notify),
                   NULL);
